...
- Hardened URL uploads against DNS rebinding: `safeFetch` now pins the connection to the addresses it validated for each hop instead of handing the hostname to a client that resolves it again at connect time. TLS SNI, the certificate check, and the `Host` header still use the original hostname. Address validation also now blocks IPv4-mapped IPv6 in hex form (`::ffff:7f00:1`), NAT64 addresses that embed a private IPv4 (`64:ff9b::/96`), 6to4 relays, `100::/64`, and the full multicast and broadcast ranges
