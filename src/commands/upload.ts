...
export async function uploadCommand(
  ...
  if (!isUploadCollection(payload, slug)) {
    ...
    const uploadCollections = collections.filter((c) =>
      isUploadCollection(payload, c)
    );
    if (uploadCollections.length > 0) {
      console.error(
        `Upload-enabled collections: ${uploadCollections.join(", ")}`
      );
    } else {
      console.error(
        "No upload-enabled collections found in this Payload config."
      );
    }
    process.exit(1);
  }
  ...
  if (filePaths.length === 0) {
    ...
    console.error(
      `Usage: payload-agent upload ${slug} <file|dir> [--data '{...}']`
    );
    process.exit(1);
  }
  ...
  if (opts.dryRun) {
    ...
    for (const filePath of resolvedFiles) {
      ...
      console.log(
        `  ${file.name} (${file.mimetype}, ${formatFileSize(file.size)})`
      );
    }
  ...
  for (let i = 0; i < resolvedFiles.length; i++) {
    ...
    try {
      ...
      if (isBulk) {
        console.error(
          `  [${i + 1}/${resolvedFiles.length}] Failed to read: ${filePath} -- ${msg}`
        );
      } else {
    ...
    try {
      ...
      if (isBulk) {
        console.error(
          `  [${i + 1}/${resolvedFiles.length}] Failed: ${file.name} -- ${msg}`
        );
      } else {
        console.error(msg);
        process.exit(1);
