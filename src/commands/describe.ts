...
function describeCollection(payload: Payload, slug: string, opts: Partial<OutputOptions>): void {
  ...
  const meta: string[] = []
  if (collection.auth) meta.push('auth')
  if (collection.upload) meta.push('upload')
  if (collection.timestamps !== false) meta.push('timestamps')
  if (meta.length > 0) lines.push(`Flags: ${meta.join(', ')}`)
